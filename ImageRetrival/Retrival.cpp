...
Mat Retrival::showResult(const vector<RetrivalResult>& result)
{
	...
	line(showImg, Point(0, rows * height - 1), Point(width * cols, rows * height - 1), Scalar(0, 0, 0));
	//垂直分割线
	for (int col = 0; col <= cols; col++)
	{
		line(showImg, Point(col * width, 0), Point(col * width, height * rows), Scalar(0, 0, 0));
	}
	line(showImg, Point(cols * width - 1, 0), Point(cols * width - 1, height * rows), Scalar(0, 0, 0));
